...
public class ResearchFileByDate {
	...
	public ResearchFileByDate(PrincipalPanel panel){
		...
		con.connect();
		String query="SELECT * FROM Filegts WHERE date like '%"+panel.getMenuTools().getSearch().getText()+"%';";
			...
			while(rs.next()){
				...
				panel.getFileResearch().addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent e) {
						...
						try{
							ResearchFileByDate.this.panel.printFile(ResearchFileByDate.this.panel.getFileResearch().getText());
						}catch(Exception a){
